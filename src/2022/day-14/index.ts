...
function partOne(wallPoints: Set<PointKey>, bounds: CavernBounds) {
  const result = particlesUntilOneFallsOutOfBounds(wallPoints, bounds, 500, 0);

  return result;
}

function partTwo(wallPoints: Set<PointKey>, bounds: CavernBounds) {
...
async function main() {
  ...
  const wallPoints = new Set<PointKey>();
