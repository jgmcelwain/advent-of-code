...
const testRebootSteps = parseRebootSteps(`on x=-5..47,y=-31..22,z=-19..33
...
describe('doCuboidsIntersect', () => {
  it.each<{ a: CuboidBounds; b: CuboidBounds; result: boolean }>([
    ...
    'identifies whether two cuboids have an intersection',
    ({ a, b, result }) => {
      expect(doCuboidsIntersect(a, b)).toBe(result);
    },
  );
});
...
describe('countActivePoints', () => {
  it('counts how many individual points are on in an array of on/off cuboids', () => {
    const result = executeRebootSteps(testRebootSteps);

    expect(countActivePoints(result)).toBe(2758514936282235);

    expect(
      countActivePoints(result, {
        x: { min: -50, max: 50 },
        y: { min: -50, max: 50 },
        z: { min: -50, max: 50 },
      }),
